...
def index_redirect():
    ...
    redirect('/tBot/index.html')


@app.route('/tBot/<file_path:path>')
def static_ardublockly(file_path):
    """Serve the 'tBot' folder static files.

    :param file_path: File path inside the 'tBot' folder.
    :return: Full HTTPResponse for the static file.
    """
    return static_file(file_path,
                       root=os.path.join(document_root, 'tBot'))
