...
def find_ardublockly_dir(search_path):
    ...
    while path_to_navigate:
        # Check if file tBot/index.html exists within current path
        if os.path.isfile(
                os.path.join(path_to_navigate, 'tBot', 'index.html')):
    ...
    if len(sys.argv) == 1:
        ...
        for opt, arg in opts:
            ...
            if opt in ('-s', '--serverroot'):
                ...
                print('Parsed "%s" flag. The tBot project root will be '
...
def main():
    ...
    # Based on command line options, set the server root to the tBot
    ...
    if find_project_root is True or server_root is None:
        ...
        # Arguments have set a server root, and to not find tBot dir
